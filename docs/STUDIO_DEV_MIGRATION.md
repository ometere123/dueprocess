...
SemanticProbe proved the `run_nondet_default` architecture with a real `SATISFIED` result. The earlier malformed CLI argument incident is operator/tooling evidence only. The repository now pins `genvm-linter==0.11.1rc2`; its current static table still reports the `run_nondet_default` E010 reachability diagnostic and stale `@allow_storage` diagnostics against the v0.3 `@gl.storage.allow` decorator. The live SemanticProbe and final lifecycle are the runtime proof. Local cached-SDK and access failures are tooling limitations, not contract behavior failures.
